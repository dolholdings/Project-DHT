...
export const KanbanView: React.FC = () => {
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    const nonBoardTasks = tasks.filter((t) => {
      ...
      if (selectedListFilter && t.listName !== selectedListFilter) return true;
      return false;
    });
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 overflow-x-auto pb-4 kanban-board-grid">
            {statuses.map((status) => {
              ...
              return (
                <div
                  key={status}
                  className={`p-4 rounded-2xl border flex flex-col h-[680px] space-y-3 shadow-md kanban-column ${getStatusHeaderAccent(
                    status
                  )} ${
                      ...
                      >
                        {colTasks.length === 0 ? (
                          <div className={`h-full min-h-[180px] rounded-xl border-2 border-dashed flex flex-col items-center justify-center p-4 text-center transition-all ${
                            ...
                            <p className="text-[10px] text-slate-500 mb-3">Drag items here or create a new deliverable.</p>
                            <button
